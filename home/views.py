    ...
    if request.method == "POST":
        try:
            ...
            with open(csv_file, 'w', newline='') as file:
                ...
                for State,WeightedAverage,Rank in zip(context['State'],context['WeightedAverage'],context['Rank']):
                    writer.writerow([State,WeightedAverage,Rank])
...
def send_email_with_pdf(request):
    if request.method == "POST":
        ...
        context={"State":[d1['State'].tolist(),d2['State'].tolist(),d3['State'].tolist()],
                 "WeightedAverage":[d1['WeightedAverage'].tolist(),d2['WeightedAverage'].tolist(),d3['WeightedAverage'].tolist()]

                 }
        csv_file = 'states_data1.csv'
        ...
        with open(csv_file, 'w', newline='') as file:
            writer = csv.writer(file)
                ...
                # Write header
            writer.writerow(['State', 'WeightedAverage'])

            for State,WeightedAverage in zip(context['State'],context['WeightedAverage']):
                writer.writerow([State,WeightedAverage])
...
def get_csv(request):
    csv_file_path = 'states_data.csv'
    with open(csv_file_path, 'r') as file:
        csv_content = file.read()
    return JsonResponse({'csv_content': csv_content})
def get_csv1(request):
    csv_file_path = 'states_data1.csv'
    with open(csv_file_path, 'r') as file:
        csv_content = file.read()
    return JsonResponse({'csv_content': csv_content})